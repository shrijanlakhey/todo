...
function App() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => getApi(current_url), 250);
  ...
  function addTodo() {
    axios({
      ...
      .catch((err) =>
        console.log(
          `Error occurred while adding todo to the server... Error: ${err}`
        )
      );
    setInputData("");
  }
  ...
  async function updateTodo(id, title, completed) {
    axios({
      ...
      .then(() => {
        // after the patch request is fulfilled call the getApi and set the todos state...
        const current_url = "http://127.0.0.1:8000/api/task-list/";
        getApi(current_url);
      })
  ...
  function deleteTodoFromAPI(id) {
    axios({
      method: "delete",
      url: `http://127.0.0.1:8000/api/task-delete/${id}`,
    })
      .then((res) => {
        console.log(
          `Todo successfully deleted from the server... Response: ${res}`
        );
      })
      .catch((err) => {
        console.log(
          `Error occurred while deleting todo from the server... Error: ${err}`
        );
      });
  }
  ...
  return (
    <div className='mx-auto min-w-[200px] max-w-md md:min-w-[444px] p-2 rounded'>
      <h1 className='text-3xl font-bold mb-4 text-white text-center bg-[#0ea5e9] py-2 rounded cool-shadow'>
        To-do
      </h1>
      <Input
      ...
      />
      {todos.length !== 0 && (
        <div className='bg-white py-1 max-h-[200px] overflow-y-auto mt-5 no-scrollbar rounded'>
          <SimpleBar className='max-h-[200px] mr-1'>
            <ul className='list-inside list-decimal my-4 px-4'>{listEl}</ul>
          </SimpleBar>
        </div>
      )}
    </div>
